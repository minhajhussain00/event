...
<div class="hidden lg:flex flex-1 items-center justify-center bg-gray-50 p-12">
  <div class="max-w-2xl w-full bg-white rounded-xl border p-8">
    <!-- data?.Event name -->
    <h1 class="text-3xl font-bold mb-2">{data?.event.name}</h1>

    <!-- Date + Location -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:gap-6 text-gray-600 mb-4">
      <p class="text-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
      ...
      <div class="p-3 rounded-lg bg-gray-50">
        ...
        <span class="font-medium">{data?.event.isPublic ? "Public" : "Private"}</span>
      </div>
      <div class="p-3 rounded-lg bg-gray-50">
